// Rudolph CX LLC — Supervisorline Demo Service Worker
...
const CACHE_NAME = 'supervisorline-demo-v1';
const ASSETS = [
  '/supervisorline-demo.html',
  '/manifest-supervisorline-demo.json',
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      return cached || fetch(event.request).catch(() => {
        return caches.match('/supervisorline-demo.html');
      });
    })
  );
});
